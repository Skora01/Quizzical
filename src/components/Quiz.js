...
function Quiz() {
    ...
    /*selectAnswer functions is going through all answers in quiz and is looking for one with same name as one selected.
    If the name is the same that means he found a group of 4 answers.Then he is looking for one with same id 
    and correct the others(because only one can be checked)*/ 
            ...
            return prevQuiz.map(item => {
                return {
                        ...
                        answers : item.answers.map(answer => {
                                return answer.name.toString() === name //if something is broken it may be in this line
                                    ? answer.id === id  
